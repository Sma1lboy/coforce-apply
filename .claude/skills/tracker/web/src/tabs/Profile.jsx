      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12 }} transition={spring}
            ...
            <p className="text-[11px] text-dim mb-3">Paste resume text (PDF copy, LinkedIn, anything). Local headless Claude parses it — nothing saves until you review and hit Save profile.</p>
            ...
            <div className="flex items-center justify-between mt-3 gap-3">
              <span className={`text-[11px] ${err ? 'text-bad' : 'text-accentsoft'}`}>
                {busy ? 'Claude is reading your resume… (~30s)' : err}
              </span>
