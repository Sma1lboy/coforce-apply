    ...
    <div className="h-full flex flex-col overflow-hidden">
      <header className="flex items-baseline gap-4 px-6 py-3.5 border-b border-rule bg-paper2 shrink-0">
        ...
        <span className="text-xs text-faint ml-auto">
          {state.apps.length} tracked · {state.campaign?.jobs?.length || 0} resumes
        </span>
      </header>
      <main className="flex-1 min-h-0 flex flex-col">
        {tab === 'discover' && <Discover state={state} onChanged={reload} goReview={() => setTab('review')} />}
        {tab === 'review' && <Review state={state} onChanged={reload} />}
