...
export default function Discover({ state, onChanged, goReview }) {
          ...
          <AnimatePresence initial={false}>
            {shown.map(j => (
              ...
              >
                ...
                <button className="btn shrink-0" disabled={queueing === j.url} onClick={() => queueForResume(j)}>
                  {queueing === j.url ? 'Queueing…' : 'Build resume →'}
                </button>
              </motion.div>
            ))}
          </AnimatePresence>
