// legacy-render.mjs — the inline-rendered board page served at /legacy and the
// static-file mode. Pure: everything it needs arrives in ctx (built by board.mjs),
...
export function renderBoard(apps, ctx) {
  const { profile, instructions, prefs, runtime, runtimeLabel, filesRoot, instructionsPath, serve, listFiles } = ctx;
  const card = a => `
...
const PREFS = ${JSON.stringify(prefs ?? null).replaceAll('<', '\\u003c')};
