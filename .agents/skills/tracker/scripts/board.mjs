...
function selectedAgent(config = null) {
...
const agentLabel = agent => (agent === 'codex' ? 'Codex' : 'Claude');

function agentRun(agent, mode, job) {
  if (agent === 'codex') {
...
function captureCodexSession(job, chunk, flush = false) {
  ...
  for (const line of lines) {
    try {
      ...
      if (
        ...
        const marks = [...String(event.item.text || '').matchAll(APPLY_STATUS_RE)]
          .map(match => match[1]);
        job.statusMarks.push(...marks);
      }
    } catch {
      // Keep raw output in the log; only JSONL thread events matter here.
    }
  }
}

function spawnAgent(job, mode, extraLog) {
  const { bin, args } = agentRun(job.agent, mode, job);
  if (job.agent === 'codex') job.statusMarks = [];
  ...
  child.stdout.on('data', d => {
    if (job.agent === 'codex') captureCodexSession(job, d.toString());
    append(d);
  });
...
// Prompt for the configured background-agent resume import (POST /api/import)
...
function runAgentImport(agent, text) {
...
function renderProfile(p) {
  if (!p)
    return '<div class="pane-empty">No profile yet — run the profile or setup skill to create it</div>';
...
function render(apps, profile = loadProfile(), instructions = readText(instructionsPath)) {
  ...
  const runtimeLabel = agentLabel(runtime);
  const card = a => `
...
<dialog id="import-dlg">
  ...
  <div class="dlg-body">
    ...
    headless ${runtimeLabel} parses it into your profile; nothing is saved until you review and hit Save.</p>
    ...
    <div class="editor-bar">
      ...
      <button id="import-run" type="button">Parse with ${runtimeLabel}</button>
    </div>
  </div>
</dialog>
<script>
...
const AGENT = ${JSON.stringify(runtime)};
const AGENT_LABEL = ${JSON.stringify(runtimeLabel)};
...
function renderList() {
  ...
  dList.innerHTML = shown.map(j => {
    ...
    return '<div class="drow">' + logo +
      ...
      '<button class="dapply" type="button" data-i="' + i + '">Build resume ⇢</button></div>';
...
dList.addEventListener('click', async e => {
  ...
  try {
    ...
    btn.disabled = true;
    queuedDirty = true;
    dStatus.textContent = 'Queued "' + job.role + '" for its grounded resume campaign. Run the start skill, then review the PDF in the full React console.';
  } catch (err) {
...
document.getElementById('import-run').addEventListener('click', async () => {
  ...
  importStatus.textContent = AGENT_LABEL + ' is reading your resume… (~30s)';
    ...
    if (req.url === '/api/import' && req.method === 'POST') {
      readBody(req, res, body => {
        try {
          ...
          const out = runAgentImport(agent, text);
