...
const templateForResumeLanguage = (dataDir, template, language) => {
  ...
  if (templateHeader === null) return template;
  let localizedHeader = templateHeader;
  const headerEscapes = {
    '\\': '\\textbackslash{}',
    '&': '\\&',
    '%': '\\%',
  ...
  const escapeHeaderText = value => String(value)
    .replace(/[\\&%#_$~^{}]/g, character => headerEscapes[character]);

  const replaceHref = (schemes, value) => {
    if (!value) return;
    const escapedSchemes = schemes.join('|');
    localizedHeader = localizedHeader.replace(
      new RegExp(`\\\\href\\{(${escapedSchemes}):[^{}]*\\}\\{[^{}]*\\}`, 'g'),
      (_, scheme) => `\\href{${scheme}:${value}}{${escapeHeaderText(value)}}`,
    );
  };
  if (contact.phone) {
    for (const scheme of ['sms', 'tel']) {
      localizedHeader = localizedHeader.replaceAll(
        ...
        `\\href{${scheme}:${contact.phone}}{${escapeHeaderText(contact.phone)}}`,
      );
    }
  }
  if (contact.email) {
    localizedHeader = localizedHeader.replaceAll(
      ...
      `\\href{mailto:${contact.email}}{${escapeHeaderText(contact.email)}}`,
    );
  }
  ...
  return template.replace(templateHeader, localizedHeader);
};
