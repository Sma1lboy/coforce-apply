...
> **Third-party skill, adapted for CoForce Apply.** Upstream:
> [LiuMengxuan04/shushu-internship-tool](https://github.com/LiuMengxuan04/shushu-internship-tool)
> (Apache-2.0, LICENSE in this directory). Original content below is unchanged
> except this notice and the "CoForce Apply Integration" section at the end.
> Scripts need `python3` (stdlib only, 3.9+ verified).
...
- `scripts/github_evidence.py`：通过认证的 `gh` 拉取归属于用户的 PR/commit 历史，生成带来源 URL 和稳定 evidence ID 的本地证据库。
- `scripts/github_sources.py`：自动合并当前账号可访问仓库与 authored-PR 仓库，生成 `github-sources.json`，无需手写 repo 清单。
...
- `references/github-evidence-library.md`：GitHub 证据库配置、隐私边界、writer contract 与校验命令。

## CoForce Apply Integration（二次创作层 — added by CoForce Apply）

在 CoForce Apply 环境（存在 `~/.coforce/`）下运行时，按以下规则与本地设施联动；
上游 workflow 不变，只是输入输出接到 CoForce 的数据面：

1. **Intake 从 profile 读，不重复问**：第 1 步的"用户基础 / 简历现状"直接读
   `~/.coforce/profile.json`（skills、experience、projects、education）；只补问
   profile 里没有的（时间预算、资源条件、运行深度）。
2. **结构化控件**：`request_user_input` 在 Claude Code 下对应 AskUserQuestion —
   taste gate 的 yes/no 和三选项都用它弹出，不在正文里模拟。
3. **尊重 standing instructions**：开始前读 `~/.coforce/instructions.md`；
   `## never-apply` 公司的 JD 直接停下告知，偏好（语言/方向/时间）并入 taste
   生成建议选项。
4. **产出写回**：
   - 最终 4-5 行 STAR 简历项目 → 经用户确认后并入 `~/.coforce/profile.json`
     的 `projects[]`（结构遵循 `profile` skill：`description` 为
     `[{text, weight?}]` 对象数组）。
   - 若该项目是为 tracker 中某个 JD 准备的，interview pack 与 audit 报告归档到
     `~/.coforce/applications/<id>/`（`tracker` skill 的档案约定），并在该申请
     `history` 里记一条 `"built ownership project: <repo>"`。
   - 与 `repo-bullets` 的分工：本 skill 负责"从 JD 造项目"；改造完成后的仓库可
     再交给 `repo-bullets` 从真实 commits 提炼更硬的 bullets。
5. **诚实边界沿用 CoForce 规则**：不为简历行发明未发生的指标；改造没做完就写
   工程产出 + 下一步计划，与上游 Operating Principle 一致。
